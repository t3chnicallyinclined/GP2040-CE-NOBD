...
**And this is not new — someone finally measured it.** Independent testing by VodkaGobalsky (a relay rig pressing two buttons at fixed delays, with a direct-to-arcade baseline as the control) found that older, slower-polling fightstick boards register simultaneous inputs *more* consistently than newer fast ones — the Xbox 360 TE even beat the arcade itself — and that boards at the *same* USB speed perform differently. That means fightstick hardware has been quietly conditioning your input for years, invisibly, inside closed firmware nobody can read. A board being more consistent at simultaneous inputs is the historical norm, not a violation of it. So "if it performs better than a Brook it must be doing something outside the scope of a normal PCB" has the history backwards: the good old boards already performed better — NOBD is just the open, deliberate version of something the boards people already trust have been doing in the dark. (Community measurement; the test pins it on poll rate, but same-speed boards differing means firmware conditioning is also at play — exact mechanism unknowable because those boards are closed.)
